...
def graph_stats(data, edges_dic_num, conv_indices_to_keep, my_new_mask_idx):
    ...
    # Compute graph depth using BFS longest path
    depths = []
    for node in G.nodes:
    ...
    # Compute width as max nodes at a single depth level
    width_levels = []
    for node in G.nodes:
    ...
    # Compute depth from root (initial node) to `my_new_mask_idx`
...
if __name__ == "__main__":
    # Example usage
    # Assuming graph_dataset is a list of Data objects from PyG
    train_loader, valid_loader, test_loader = get_pyg_data_loaders("cad", True, 0) #get_data_loaders(size, validation)
